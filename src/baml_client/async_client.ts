...
export class BamlAsyncClient {
  ...
  async ChatWithLLM(
    ...
    try {
      ...
      return raw.parsed(false) as AssistantResponse
    } catch (error) {
...
class BamlStreamClient {
  ...
  ): BamlStream<partial_types.AssistantResponse, AssistantResponse> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      const collector = options.collector ? (Array.isArray(options.collector) ? options.collector : [options.collector]) : [];
      ...
      return new BamlStream<partial_types.AssistantResponse, AssistantResponse>(
        raw,
        (a): partial_types.AssistantResponse => a,
        (a): AssistantResponse => a,
        this.ctxManager.cloneContext(),
      )
    } catch (error) {
      throw toBamlError(error);
